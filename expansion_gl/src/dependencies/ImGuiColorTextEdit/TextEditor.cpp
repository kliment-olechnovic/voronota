	...
	, mFocused(false)
{
	...
	SetLanguageDefinition(LanguageDefinition::Simple());
...
void TextEditor::MoveRight(int aAmount, bool aSelect, bool aWordMode)
{
	...
	if (mLines.empty() || oldPos.mLine >= static_cast<int>(mLines.size()))
		return;
	...
	while (aAmount-- > 0)
	{
		...
		{
			if (mState.mCursorPosition.mLine < static_cast<int>(mLines.size()) - 1)
			{
...
void TextEditor::Backspace()
{
	...
	else
	{
		...
		else
		{
			...
			while (cindex < static_cast<int>(line.size()) && cend-- > cindex)
			{
				u.mRemoved += line[cindex].mChar;
				line.erase(line.begin() + cindex);
			}
...
float TextEditor::TextDistanceToLineStart(const Coordinates& aFrom) const
{
	...
	for (size_t it = 0u; it < line.size() && static_cast<int>(it) < colIndex; )
	{
		if (line[it].mChar == '\t')
		{
...
static bool TokenizeCStyleBrackets(const char * in_begin, const char * in_end, const char *& out_begin, const char *& out_end)
{
	(void)in_end;
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Simple()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
	{
		langDef.mTokenize = [](const char * in_begin, const char * in_end, const char *& out_begin, const char *& out_end, PaletteIndex & paletteIndex) -> bool
		{
			...
			if (in_begin == in_end)
			{
				...
				paletteIndex = PaletteIndex::String;
			else if (TokenizeCStyleBrackets(in_begin, in_end, out_begin, out_end))
				paletteIndex = PaletteIndex::Punctuation;
