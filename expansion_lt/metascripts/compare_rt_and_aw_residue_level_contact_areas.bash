...
cat "${TMPLDIR}/balls" | ./voronota-lt -probe 1.4 -print-contacts-residue-level > "${TMPLDIR}/areas_rt"

cat "${TMPLDIR}/balls" | ./voronota-lt -probe 1.4 -print-contacts-residue-level -run-in-aw-diagram-regime > "${TMPLDIR}/areas_aw"
...
dt=merge(dt1, dt2, by=c("ID1_chain","ID1_residue","ID2_chain","ID2_residue"), all=TRUE);
...
plot(x=dt$area.x, y=dt$area.y, xlab="area AW", ylab="area RT", main="AW vs RT residue-level areas", col=densCols(dt$area.x, dt$area.y));
