...
Options:
    ...
    --graphics-color-wireframe                       string     hex-coded color for wireframe and sasmesh, default is '0x808080'
    --graphics-color-sas                             string     hex-coded color for SAS, default is '0x00FF00'
...
public:
	...
	unsigned int graphics_color_sas;
	...
	bool read_from_command_line_args(const int argc, const char** argv) noexcept
	{
...

		{
			...
			for(std::size_t i=0;i<cloptions.size();i++)
			{
				...
				else if(opt.name=="graphics-color-sas" && opt.args_hexints.size()==1)
				{
					graphics_color_sas=opt.args_hexints.front();
				}
		...
		ApplicationMeshRecorder& app_mesh_recorder) noexcept
{
...

	{
		...
		if(!result.cells_summaries.empty() && !app_params.filtering_expression_for_restricting_cells_for_output.allow_all())
		{
	...
	if(app_graphics_recorder.graphics_writer.enabled())
	{
		...
		if(ApplicationGraphicsRecorder::allow_representation(app_params.graphics_restrict_representations, "sas"))
		{
			...
			{
				app_graphics_recorder.graphics_writer.add_color("sas", "", app_params.graphics_color_sas);
				for(std::size_t i=0;i<result_graphics.sas_graphics.size();i++)
				{
					...
					for(std::size_t j=0;j<gb.triples.size();j++)
					{
						...
						app_graphics_recorder.graphics_writer.add_triangle_on_sphere("sas", groupname, spheres_input_result.spheres[sphere_id].p, gb.vertices[t.ids[0]], gb.vertices[t.ids[1]], gb.vertices[t.ids[2]]);
					}
				}
			}
		}
				...
				for(std::size_t i=0;i<result_graphics.sas_graphics.size();i++)
				{
					...
					if(!gb.empty())
					{
						...
						for(std::size_t j=0;j<pairs.size();j++)
						{
							...
							app_graphics_recorder.graphics_writer.add_line_strip("sasmesh", groupname, strip);
						}
					}
				}
			}
