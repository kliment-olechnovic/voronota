...
public:
	...
	static bool slice_spheres_input_result(const bool select_all, const std::vector<std::size_t>& indices, Result& result)
	{
		if(select_all)
		{
			return !result.spheres.empty();
		}
		if(indices.empty() || result.spheres.empty())
		{
			return false;
		}
		...
		if(!result.sphere_labels.empty())
		{
			sub_result.sphere_labels.reserve(indices.size());
		}
		if(!result.grouping_by_chain.empty())
		{
			sub_result.grouping_by_chain.reserve(indices.size());
		}
		if(!result.grouping_by_residue.empty())
		{
		...
		for(std::size_t i=0;i<indices.size();i++)
		{
			...
			if(id<result.spheres.size())
			{
				something_selected=true;
				sub_result.spheres.push_back(result.spheres[id]);
			}
			if(id<result.sphere_labels.size())
			{
				sub_result.sphere_labels.push_back(result.sphere_labels[id]);
			}
			if(id<result.grouping_by_chain.size())
			{
				sub_result.grouping_by_chain.push_back(result.grouping_by_chain[id]);
			}
			if(id<result.grouping_by_residue.size())
			{
				sub_result.grouping_by_residue.push_back(result.grouping_by_residue[id]);
			}
		}
		if(something_selected)
		{
			result.spheres.swap(sub_result.spheres);
			result.sphere_labels.swap(sub_result.sphere_labels);
			result.grouping_by_chain.swap(sub_result.grouping_by_chain);
			result.grouping_by_residue.swap(sub_result.grouping_by_residue);
		}
