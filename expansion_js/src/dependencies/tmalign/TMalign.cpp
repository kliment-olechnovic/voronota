    ...
    const string atom_opt, const int split_opt, const int het_opt, StandardOutputMockup& som)
{
    ...
    InputSelector input_selector(filename);
    istream& fin=input_selector.stream();
    ...
    vector<int> &mol_vec, const int ter_opt, const int split_opt)
{
    ...
    while (fin.good())
    {
        ...
        else
        {
            ...
            for (l=0;l<static_cast<int>(line.size());l++) mol_vec.back()+=
...
{
    int i;
    for (i=0;i<static_cast<int>(PDB_lines.size());i++)
    {
    ...
    const int i_opt, StandardOutputMockup& som)
{
    ...
    InputSelector input_selector(fname_lign);
    istream& fileIn=input_selector.stream();
    ...
    double score_d8, double d0)
{
    ...
    const int kmax=Lali;    
    std::vector<int> k_ali(kmax); //int k_ali[kmax];
    int ka, k;
    double t[3];
    double u[3][3];
    double d;
    ...
    std::vector<int> L_ini(n_init_max); //int L_ini[n_init_max];  //fragment lengths, Lali, Lali/2, Lali/4 ... 4
    int L_ini_min=4;
    ...
    std::vector<int> i_ali(kmax);//int i_ali[kmax];
    int n_cut;
    ...
    double d0, double d0_search, double t[3], double u[3][3], StandardOutputMockup& som)
{
    ...
    double di;
    const int len=k;
    std::vector<double> dis(len); //double dis[len];
...
double DP_iter(double **r1, double **r2, double **xtm, double **ytm,
    ...
    double /*D0_MIN8*/, double Lnorm, double d0, double score_d8)
{
    ...
    const double rmsd, const double TM1, const double Liden, StandardOutputMockup& som)
{
    ...
    int i2=-1;
    int i;
    const int strlen_seqM=strlen(seqM);
    for (i=0;i<strlen_seqM;i++)
    {
        i1+=(seqxA[i]!='-');
        i2+=(seqyA[i]!='-');
        if (seqM[i]==' ') continue;
    ...
    asym_id="";
    {
    InputSelector input_selector(xname);
    istream& fin=input_selector.stream();
    while (fin.good())
    {
        getline(fin, line);
    ...
    for (i=1;i<static_cast<int>(idx_vec.size());i++) buf<<"CONECT"
    ...
    asym_id="";
    {
    InputSelector input_selector(yname);
    istream& fin=input_selector.stream();
    while (fin.good())
    {
        getline(fin, line);
    ...
    for (i=0;i<static_cast<int>(pml_list.size());i++)
    {
    ...
    const int mol_type, const double TMcut, StandardOutputMockup& som)
{
    ...
    seqxA=seqxA_cp;
    seqyA=seqyA_cp;
    for (i=0;i<static_cast<int>(seqxA_cp.size());i++)
    {
        if (seqxA_cp[i]!='-')
        {
            seqxA[r]=seqxA_cp[i];
...
int main_of_tmalign(int argc, const char** argv, StandardOutputMockup& som)
{
    ...
    int    ychainnum=0;// number of chains in a PDB file
    ...
    for (i=0;i<static_cast<int>(chain1_list.size());i++)
    {
        /* parse chain 1 */
        xname=chain1_list[i];
        ...
        for (chain_i=0;chain_i<xchainnum;chain_i++)
        {
            ...
            for (j=(dir_opt.size()>0 ? 1 : 0)*(i+1);j<static_cast<int>(chain2_list.size());j++)
            {
                /* parse chain 2 */
