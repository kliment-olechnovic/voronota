...
void demo_apilike_functionality(const auxiliaries::ProgramOptionsHandler& poh)
{
	...
	manager.execute("print-contacts sel '{atom-first {match R<PHE>} atom-second {match R<PHE>} min-area 5.0 min-seq-sep 1}' summarize echo file tmp/contacts.txt", std::cout);
}
