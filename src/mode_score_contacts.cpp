...
{
	{
		...
		list_of_option_descriptions.push_back(OD("--erf", "", "flag to apply error function to scores"));
		...
		{
			std::cerr << "stdin   <-  list of contacts (line format: 'annotation1 annotation2 area')\n";
			std::cerr << "stdout  ->  global score value\n";
			return;
		}
	}
	...
	std::map< std::pair<CRAD, CRAD>, EnergyDescriptor > inter_atom_scores;
	{
		for(std::map< std::pair<CRAD, CRAD>, double >::iterator it=map_of_contacts.begin();it!=map_of_contacts.end();++it)
		{
			const std::pair<CRAD, CRAD>& crads=it->first;
			EnergyDescriptor& ed=inter_atom_scores[crads];
	...
	std::map< std::pair<CRAD, CRAD>, EnergyDescriptor > inter_residue_scores;
	{
		for(std::map< std::pair<CRAD, CRAD>, EnergyDescriptor >::const_iterator it=inter_atom_scores.begin();it!=inter_residue_scores.end();++it)
		{
			const std::pair<CRAD, CRAD>& crads=it->first;
			inter_residue_scores[modescommon::refine_pair_by_ordering(std::make_pair(crads.first.without_atom(), crads.second.without_atom()))].add(it->second);
		}
	}

	std::map<CRAD, EnergyDescriptor> residue_scores;
	{
		std::map< CRAD, std::set<CRAD> > residue_graph;
		for(std::map< std::pair<CRAD, CRAD>, EnergyDescriptor >::const_iterator it=inter_residue_scores.begin();it!=inter_residue_scores.end();++it)
		{
			const std::pair<CRAD, CRAD>& crads=it->first;
		...
		for(std::map< std::pair<CRAD, CRAD>, EnergyDescriptor >::const_iterator it=inter_residue_scores.begin();it!=inter_residue_scores.end();++it)
		{
			...
			{
				residue_scores[*jt].add(it->second);
			}
		}
	}
}
