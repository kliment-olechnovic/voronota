...
public:
	...
	ExportSequence() : not_fill_middle_gaps(false), not_fill_start_gaps(false)
	{
	}
	...
	void document(CommandDocumentation& doc) const
	{
		...
		doc.set_option_decription(CDOD("not-fill-middle-gaps", CDOD::DATATYPE_BOOL, "flag to not fill middle gaps with X"));
		doc.set_option_decription(CDOD("not-fill-start-gaps", CDOD::DATATYPE_BOOL, "flag to not fill start gaps with X"));
	}
	...
	Result run(DataManager& data_manager) const
	{
		...
		const std::map<std::string, std::string> chain_sequences=common::ConstructionOfPrimaryStructure::collect_chain_sequences_from_atom_ids(data_manager.primary_structure_info(), ids, !not_fill_middle_gaps, !not_fill_start_gaps);
