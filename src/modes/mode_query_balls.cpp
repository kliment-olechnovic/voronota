...
void query_balls(const voronota::auxiliaries::ProgramOptionsHandler& poh)
{
	...
	if(update_mode && !selected_set_of_ball_ids.empty())
	{
		...
		const std::map<CRAD, int> sequence_mapping=voronota::common::SequenceUtilities::construct_sequence_mapping(residue_sequence_vector, reference_sequence, false, ref_seq_alignment);
