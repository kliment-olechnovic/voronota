#ifndef MODESCOMMON_HANDLE_SEQUENCES_H_
#define MODESCOMMON_HANDLE_SEQUENCES_H_
...
inline std::string read_sequence_from_file(const std::string& filename)
{
	std::string result;
	if(!filename.empty())
	{
...
template<typename T>
inline std::string convert_residue_sequence_container_to_string(const T& residue_sequence_container)
{
	std::string seq;
...
inline std::map<auxiliaries::ChainResidueAtomDescriptor, double> construct_sequence_mapping(const std::vector<auxiliaries::ChainResidueAtomDescriptor>& residue_sequence_vector, const std::string& reference_sequence, const std::string& ref_seq_alignment_output_filename)
{
...
#endif /* MODESCOMMON_HANDLE_SEQUENCES_H_ */
