...
then
	...
	$BINDIR/schedule_jobs.bash -b $BINDIR -s $SCHEDULER -p $CPUCOUNT -c "$BINDIR/calc_contacts_and_summary_from_raw_contacts.bash -d" -a $OUTPUTDIR/list_of_entries_with_raw_contacts
	exit 0
fi
...
then
	...
	$BINDIR/schedule_jobs.bash -b $BINDIR -s $SCHEDULER -p $CPUCOUNT -c "$BINDIR/calc_energies_from_contacts_and_potential.bash -p $BINDIR/potential -d" -a $OUTPUTDIR/list_of_entries_with_contacts
	exit 0
fi
...
then
	...
	$BINDIR/schedule_jobs.bash -b $BINDIR -s $SCHEDULER -p $CPUCOUNT -c "$BINDIR/calc_quality_scores_from_energies.bash -e $BINDIR/energy_means_and_sds -d" -a $OUTPUTDIR/list_of_entries_with_atom_energies
	exit 0
fi
...
then
	find $OUTPUTDIR/entries/ -type f -name balls -not -empty | sed 's/balls$//' > $OUTPUTDIR/list_of_entries_with_balls
	$BINDIR/schedule_jobs.bash -b $BINDIR -s $SCHEDULER -p $CPUCOUNT -c "$BINDIR/collect_atoms_from_balls.bash -d" -a $OUTPUTDIR/list_of_entries_with_balls
	exit 0
fi
...
then
	...
	$BINDIR/schedule_jobs.bash -b $BINDIR -s $SCHEDULER -p $CPUCOUNT -c "$BINDIR/calc_goap_scores_from_atoms.bash -d" -a $OUTPUTDIR/list_of_entries_with_atoms
	exit 0
fi
...
then
	find $OUTPUTDIR/entries/ -type f -name atoms.pdb -not -empty | sed 's/atoms.pdb$//' > $OUTPUTDIR/list_of_entries_with_atoms
	$BINDIR/schedule_jobs.bash -b $BINDIR -s $SCHEDULER -p $CPUCOUNT -c "$BINDIR/calc_rwplus_score_from_atoms.bash" -a $OUTPUTDIR/list_of_entries_with_atoms
	exit 0
fi
...
then
	find $OUTPUTDIR/entries/ -type f -name atoms.pdb -not -empty | sed 's/atoms.pdb$//' > $OUTPUTDIR/list_of_entries_with_atoms
	$BINDIR/schedule_jobs.bash -b $BINDIR -s $SCHEDULER -p $CPUCOUNT -c "$BINDIR/calc_doop_score_from_atoms.bash" -a $OUTPUTDIR/list_of_entries_with_atoms
	exit 0
fi
...
then
	...
	$BINDIR/schedule_jobs.bash -b $BINDIR -s $SCHEDULER -p $CPUCOUNT -c "$BINDIR/calc_cad_scores_from_model_and_target_contacts.bash -d" -a $OUTPUTDIR/list_of_entries_with_models_contacts
	exit 0
fi
...
then
	...
	$BINDIR/schedule_jobs.bash -b $BINDIR -s $SCHEDULER -p $CPUCOUNT -c "$BINDIR/calc_tmscore_from_model_and_target_atoms.bash -d" -a $OUTPUTDIR/list_of_entries_with_models_atoms
	exit 0
fi
...
then
	...
	$BINDIR/schedule_jobs.bash -b $BINDIR -s $SCHEDULER -p $CPUCOUNT -c "$BINDIR/collect_scores_list_from_working_directory.bash -d" -a $OUTPUTDIR/list_of_entries_with_global_quality_score
	exit 0
fi
...
then
	...
	$BINDIR/schedule_jobs.bash -b $BINDIR -s $SCHEDULER -p $CPUCOUNT -c "$BINDIR/calc_local_scores_evaluation_from_target_models_local_scores.bash -d" -a $OUTPUTDIR/list_of_target_directories
